export default {
  ...
  _updateVoiceButtons(inVoice) {
    ...
    if (!inVoice) {
      ...
      if (_ltnBtn) {
        _ltnBtn.disabled = false;
        _ltnBtn.title = t("voice.panel.listen_together");
      }
      ...
      if (wcGrid) {
        wcGrid.querySelectorAll("video").forEach((v) => {
          v.srcObject = null;
        });
        wcGrid.innerHTML = "";
      }
    ...
    if (document.pictureInPictureEnabled && !video.disablePictureInPicture) {
      video
        ...
        .then(() => {
          ...
          if (popoutBtn) {
            popoutBtn.textContent = "⧈";
            popoutBtn.title = t("media.pop_in_camera");
          }
  ...
  _handleScreenStream(userId, stream, { force = false } = {}) {
    ...
    if (stream) {
      ...
      if (!autoAccept && userId !== null && userId !== this.user.id) {
        ...
        this._showToast(
          t("voice.sharing_started", { who: this._escapeHtml(who) }),
          "info",
          {
            label: "Join",
    ...
    if (document.pictureInPictureEnabled && !video.disablePictureInPicture) {
      video
        ...
        .then(() => {
          ...
          if (popoutBtn) {
            popoutBtn.textContent = "\u29C8";
            popoutBtn.title = t("media.pop_in_stream");
          }
  ...
  _getYouTubePlaylistInfo(url) {
    ...
    try {
      ...
      const videoId = host === "youtu.be" ? parsed.pathname.replace(/^\/+/, "").split("/")[0] : parsed.searchParams.get("v");
  ...
  _previewMusicLink(url) {
    ...
    if (playlistInfo) {
      //Conditional display of playlist parsing
  ...
  _shareMusic() {
    ...
    if (!url) {
      this._showToast(t("toasts.paste_music_link"), "error");
      return;
    }
    ...
    if (!this.voice || !this.voice.inVoice) {
      this._showToast(t("toasts.join_voice_required"), "error");
      return;
    }
  ...
  _shareMusicPlaylist() {
    ...
    if (!info?.playlistId) {
      this._showToast("No playlist found in this link", "error");
      return;
    }
    if (!this.voice || !this.voice.inVoice) {
  ...
  _stopMusic() {
    //Check for music management role to halt playback
  ...
  _handleMusicShared(data) {
    ...
    if (isSpotify) {
      if (volSlider) {
        ...
        volSlider.title = t("media.spotify_volume_hint");
      }
      if (muteBtn) {
        muteBtn.disabled = true;
        muteBtn.title = t("media.spotify_volume_hint");
      }
    } else {
      if (volSlider) {
      ...
      if (muteBtn) {
        ...
        muteBtn.title = t("media.mute_unmute");
      }
    }
  ...
  _canControlMusic() {
    return this.user?.isAdmin || this._musicRequestorId === this.user?.id || this._hasPerm("manage_music_queue");
  },
  ...
  _syncMusicQueueUi() {
    const text = this._musicUpNext?.title ? `Up next: ${this._truncateMusicQueueTitle(this._musicUpNext.title, 54)}` : "Up next: Nothing queued";
  ...
  _renderMusicQueueModal() {
    ...
    summary.textContent = queue.length ? `${queue.length} queued track${queue.length === 1 ? "" : "s"}` : "No queued tracks";
    if (!queue.length) {
  ...
  _createSCWidget(iframe, volume) {
    try {
      ...
      this._musicSCWidget.bind(SC.Widget.Events.PLAY, () => {
        this._setMusicPlayingUi(true);
        if (!this._shouldSuppressMusicBroadcasts()) this._emitMusicControl("play");
        this._musicSCWidget.getCurrentSoundIndex((idx) => {
          this._musicSCCurrentIndex = idx;
        });
      });
  ...
  _musicToggleShuffle() {
    try {
      ...
      /* player may not support shuffle */
    }
  },
  ...
  _popInMusicPlayer() {
    ...
    this._captureCurrentMusicSyncState((syncState) => {
      ...
      if (popBtn) {
        popBtn.textContent = "⧉";
        popBtn.title = t("media.music_popout");
      }
    });
  },
  ...
  _stopMusicTimeTracking() {
    if (this._musicTimeInterval) {
      clearInterval(this._musicTimeInterval);
      this._musicTimeInterval = null;
    }
    const seekSlider = document.getElementById("music-seek-slider");
    const curEl = document.getElementById("music-time-current");
    const durEl = document.getElementById("music-time-duration");
  ...
  _getMusicEmbed(url) {
    ...
    if (ytMusicMatch)
      return `https://www.youtube-nocookie.com/embed/${ytMusicMatch[1]}?autoplay=1&enablejsapi=1&origin=${encodeURIComponent(location.origin)}&rel=0`;
    const ytMatch = url.match(/(?:youtube\.com\/watch\?v=|youtu\.be\/|youtube\.com\/shorts\/)([a-zA-Z0-9_-]{11})/);
    if (ytMatch)
      return `https://www.youtube-nocookie.com/embed/${ytMatch[1]}?autoplay=1&enablejsapi=1&origin=${encodeURIComponent(location.origin)}&rel=0`;
