export default {
  ...
  _setupUI() {
    ...
    document.querySelector('#channel-ctx-menu [data-action="mute"]')?.addEventListener("click", () => {
      ...
      if (idx >= 0) {
        muted.splice(idx, 1);
        this._showToast(t("toasts.channel_unmuted"), "success");
      } else {
        muted.push(code);
        this._showToast(t("toasts.channel_muted"), "success");
      }
    ...
    document.getElementById("channel-functions-panel")?.addEventListener("click", (e) => {
      ...
      if (fn === "streams") {
        ...
        if (newVal === 0) {
          patch.streams_enabled = 0;
          patch.music_enabled = 0;
        }
        optimistic(patch);
        ...
        input.min = "2";
        input.max = "99";
        input.value = current >= 2 ? current : "";
        input.placeholder = t("channel_functions.voice_limit_placeholder");
        input.className = "cfn-input";
        input.onclick = (e2) => e2.stopPropagation();
        badge.replaceWith(input);
        input.focus();
        ...
        input.placeholder = t("channel_functions.bitrate_placeholder");
        input.className = "cfn-input";
        input.onclick = (e2) => e2.stopPropagation();
        badge.replaceWith(input);
        input.focus();
        ...
        const commitBitrate = () => {
          ...
          let bitrate = 0;
          if (!isNaN(raw) && raw > 0) {
            bitrate = validBitrates.reduce((prev, curr) => (Math.abs(curr - raw) < Math.abs(prev - raw) ? curr : prev));
          }
        ...
        input.max = "720";
        input.value = "";
        input.placeholder = t("channel_functions.self_destruct_placeholder");
        input.className = "cfn-input";
        input.onclick = (e2) => e2.stopPropagation();
        badge.replaceWith(input);
        input.focus();
        ...
        input.max = "1440";
        input.value = current > 0 ? current : "";
        input.placeholder = "1–1440 (0=off)";
        input.className = "cfn-input";
        input.onclick = (e2) => e2.stopPropagation();
        badge.replaceWith(input);
        input.focus();
    ...
    document.getElementById("organize-global-sort")?.addEventListener("change", (e) => {
      ...
      if (this._organizeServerLevel) {
        ...
        if (parent)
          parent.sort_alphabetical =
            sortMode === "alpha" ? 1 : sortMode === "created" ? 2 : sortMode === "oldest" ? 3 : sortMode === "dynamic" ? 4 : 0;
      }
    ...
    document.getElementById("webhook-copy-url-btn")?.addEventListener("click", () => {
      ...
      const markCopied = () => {
        ...
        setTimeout(() => {
          document.getElementById("webhook-copy-url-btn").textContent = "📋 " + t("common.copy");
        }, 2000);
      };
      navigator.clipboard
    ...
    if (streamSizeSlider) {
      ...
      const applySize = () => {
        ...
        _resizeRAF = requestAnimationFrame(() => {
          ...
          document.querySelectorAll(".screen-share-tile video").forEach((v) => {
            v.style.maxHeight = vh - 4 + "vh";
          });
    ...
    if (webcamSizeSlider) {
      ...
      const applyWcSize = () => {
        ...
        _wcResizeRAF = requestAnimationFrame(() => {
          ...
          document.querySelectorAll(".webcam-tile").forEach((t) => {
            t.style.maxWidth = tileMaxW + "vw";
          });
          document.querySelectorAll(".webcam-tile video").forEach((v) => {
            v.style.maxHeight = vh - 4 + "vh";
          });
    ...
    // ── Video codec mode selector ──
    const codecModeSelect = document.getElementById("video-codec-mode-select");
    const codecDebugPanel = document.getElementById("voice-codec-debug");
    if (codecModeSelect) {
      const savedCodecMode = localStorage.getItem("haven_video_codec_mode") || "preferhwaccel";
      codecModeSelect.value = savedCodecMode === "auto" ? "auto" : "preferhwaccel";
      codecModeSelect.addEventListener("change", (e) => {
        this.voice.setVideoCodecMode(e.target.value);
      });
    }

    // Wire up codec debug state updates
    if (this.voice) {
      this.voice.onCodecDebugUpdate = (state) => {
        if (!codecDebugPanel) return;
        // Show debug panel if any video is active
        if (this.voice.isScreenSharing || this.voice.isWebcamActive) {
          codecDebugPanel.style.display = "block";
        } else {
          codecDebugPanel.style.display = "none";
        }
        // Update debug display fields
        document.getElementById("codec-debug-mode").textContent = state.mode || "-";
        document.getElementById("codec-debug-default").textContent = state.defaultCodec || "-";
        document.getElementById("codec-debug-preferred").textContent = state.preferredCodec || "-";
        document.getElementById("codec-debug-active").textContent = state.activeCodec || "-";
        document.getElementById("codec-debug-stats").textContent = state.liveStats || "-";
      };
    }
    ...
    this._gamesRegistry = [
      { id: "flappy", name: "Shippy Container", icon: "🚢", path: "/games/flappy.html", description: "Dodge containers, chase high scores!" },
      {
        ...
        name: "Flight",
        icon: "✈️",
        path: "/games/flash.html?swf=/games/roms/flight-759879f9.swf&title=Flight",
        description: "Throw a paper plane as far as you can!",
        type: "flash",
      },
      {
        id: "learn-to-fly-3",
        name: "Learn to Fly 3",
        icon: "🐧",
        path: "/games/flash.html?swf=/games/roms/learn-to-fly-3.swf&title=Learn%20to%20Fly%203",
      ...
      },
      {
        ...
        name: "Bubble Tanks 3",
        icon: "🫧",
        path: "/games/flash.html?swf=/games/roms/Bubble%20Tanks%203.swf&title=Bubble%20Tanks%203",
        description: "Bubble-based arena shooter",
        type: "flash",
      },
      {
        id: "tanks",
        name: "Tanks",
        icon: "🪖",
        path: "/games/flash.html?swf=/games/roms/tanks.swf&title=Tanks",
        description: "Classic Armor Games tank combat",
        type: "flash",
      },
      {
        ...
        icon: "⚔️",
        path: "/games/flash.html?swf=/games/roms/SuperSmash.swf&title=Super%20Smash%20Flash%202",
        description: "Fan-made Smash Bros platformer fighter",
        type: "flash",
      },
      {
        id: "io-games",
        name: ".io Games",
        icon: "🌐",
        path: "/games/io-games.html",
        description: "Browse popular .io multiplayer games",
        type: "browser",
      },
    ];
    ...
    if (!this._gameScoreListenerAdded) {
      window.addEventListener("message", (e) => {
        ...
        if (e.data && e.data.type === "get-leaderboard") {
          ...
          try {
            target?.postMessage({ type: "leaderboard-data", leaderboard: scores }, e.origin);
          } catch {}
        }
      });
    ...
    document.getElementById("messages").addEventListener(
      ...
      true,
    ); // capture phase so it fires before the toolbar action handler
    ...
    document.getElementById("settings-modal").addEventListener("click", (e) => {
      ...
      if ((setupArea && setupArea.style.display !== "none") || (backupArea && backupArea.style.display !== "none")) return;
      this._cancelAdminSettings();
    });
    ...
    const loadTotpStatus = async () => {
      ...
      try {
        ...
        const data = await res.json();
        if (!res.ok) {
          totpStatusText.textContent = data.error || t("settings.two_factor_section.error");
          return;
        }
        ...
        if (data.enabled) {
          ...
          if (totpManageStatus) {
            totpManageStatus.textContent = "";
            totpManageStatus.className = "settings-hint";
          }
        } else {
          totpStatusText.textContent = "";
    ...
    document.getElementById("totp-enable-btn")?.addEventListener("click", async () => {
      ...
      if (totpSetupStatus) {
        ...
        totpSetupStatus.className = "settings-hint";
      }
      ...
      try {
        ...
        const data = await res.json();
        if (!res.ok) {
          totpSetupStatus.textContent = data.error || t("settings.two_factor_section.setup_failed");
          return;
        }
    ...
    document.getElementById("totp-verify-setup-btn")?.addEventListener("click", async () => {
      ...
      try {
        ...
        if (!res.ok) {
          if (totpSetupStatus) {
            totpSetupStatus.textContent = data.error || t("settings.two_factor_section.verify_failed");
            totpSetupStatus.classList.add("error");
          }
          return;
        }
    ...
    document.getElementById("totp-copy-backup-btn")?.addEventListener("click", () => {
      ...
      const markCopied = () => {
        ...
        setTimeout(() => {
          btn.textContent = "📋 " + t("settings.two_factor_section.copy_backup_btn");
        }, 2000);
      };
      navigator.clipboard
        .writeText(codes)
    ...
    document.getElementById("totp-disable-btn")?.addEventListener("click", async () => {
      ...
      if (!pw) {
        if (totpManageStatus) totpManageStatus.textContent = t("settings.two_factor_section.disable_prompt");
        return;
      }
      try {
        ...
        if (!res.ok) {
          if (totpManageStatus) {
            totpManageStatus.textContent = data.error || t("settings.two_factor_section.failed");
            totpManageStatus.classList.add("error");
          }
          return;
        }
    ...
    document.getElementById("totp-regen-backup-btn")?.addEventListener("click", async () => {
      const pw = document.getElementById("totp-disable-password")?.value;
      if (!pw) {
        if (totpManageStatus) totpManageStatus.textContent = t("settings.two_factor_section.regen_prompt");
        return;
      }
      try {
    ...
    const loadRecoveryStatus = async () => {
      ...
      try {
        ...
        const data = await res.json();
        if (!res.ok) {
          statusEl.textContent = data.error || t("settings.two_factor_section.error");
          return;
        }
    ...
    document.getElementById("recovery-generate-btn")?.addEventListener("click", async () => {
      ...
      if (!password) {
        statusEl.textContent = t("settings.recovery_section.confirm_prompt");
        return;
      }
      statusEl.textContent = "";
      try {
        ...
        const data = await res.json();
        if (!res.ok) {
          statusEl.textContent = data.error || t("settings.recovery_section.failed");
          return;
        }
    ...
    document.getElementById("recovery-copy-btn")?.addEventListener("click", () => {
      const codesEl = document.getElementById("recovery-codes-list");
      if (!codesEl) return;
      const text = Array.from(codesEl.querySelectorAll("div"))
        .map((d) => d.textContent)
        .join("\n");
      ...
      const markCopied = () => {
        btn.textContent = "✅ " + t("common.copied") + "!";
        setTimeout(() => {
          btn.textContent = "📋 " + t("settings.recovery_section.copy_codes_btn");
        }, 2000);
      };
      navigator.clipboard
        .writeText(text)
    ...
    `;
      ...
      overlay.querySelector(".self-delete-confirm").addEventListener("click", () => {
        ...
        if (!pw) {
          status.textContent = t("settings.delete_account_section.password_required");
          return;
        }
    ...
    document.getElementById("backup-download-btn")?.addEventListener("click", () => {
      ...
      if (
        heavy &&
        !confirm(
          t("confirm.backup_heavy") ||
            "This backup includes messages and/or uploaded files. It may take a while and produce a large download. Continue?",
        )
      )
        return;
    ...
    document.getElementById("backup-select-all-btn")?.addEventListener("click", () => {
      document.querySelectorAll(".backup-include").forEach((el) => {
        el.checked = true;
      });
    });
    ...
    if (restoreBtn) {
      restoreBtn.addEventListener("click", async () => {
        ...
        if (
          !confirm(
            t("confirm.backup_restore") ||
              "Restore this backup? It will OVERWRITE the current server data and restart the server. This cannot be undone (except via the haven.db.pre-restore copy on the host machine).",
          )
        )
          return;
        const token = localStorage.getItem("haven_token");
  ...
  _copyChannelLink(code, messageId = null) {
    ...
    if (navigator.clipboard?.writeText) {
      navigator.clipboard
        .writeText(url)
        .then(onCopied)
        .catch(() => this._copyTextFallback(url, onCopied));
    } else {
      ...
      window.havenDesktop
        ...
        .then((history) => {
          ...
          for (const h of history || []) {
            ...
            let normalizedUrl;
            try {
              normalizedUrl = new URL(h.url).origin;
            } catch {
              normalizedUrl = h.url;
            }
  ...
  _renderManageServersList() {
    ...
    const servers = this.serverManager.getAll().filter((s) => {
      ...
      try {
        return new URL(s.url).origin !== currentOrigin;
      } catch {
        return true;
      }
    });
    ...
    servers.forEach((s) => {
      ...
      const iconContent = iconUrl ? `<img src="${this._escapeHtml(iconUrl)}" alt="" class="manage-srv-icon-img">` : initial;
  ...
  _renderServerBar() {
    ...
    list.innerHTML = servers
      .map((s) => {
        ...
        const statusClass = online === true ? "online" : online === false ? "offline" : "unknown";
        const statusText =
          online === true ? "● " + t("servers.online") : online === false ? "○ " + t("servers.offline") : "◌ " + t("servers.checking");
  ...
  _setupMobile() {
    ...
    if (isTouchDevice) {
      ...
      messagesEl.addEventListener("click", (e) => {
        ...
        if (e.target.closest("a") || e.target.closest(".reaction-badge") || e.target.closest(".spoiler") || e.target.closest(".reply-banner")) return;
        // Don't interfere with author/avatar clicks (profile popup)
        if (e.target.closest(".message-author") || e.target.closest(".message-avatar") || e.target.closest(".message-avatar-img")) return;
  ...
  _updatePollRemoveButtons() {
    ...
    btns.forEach((b) => {
      b.style.display = list.children.length > 2 ? "" : "none";
    });
  },
    ...
    if (isIOS || /Android/.test(navigator.userAgent)) {
      ...
      if (isMobile) {
        ...
        requestAnimationFrame(() => {
          ...
          probe.style.cssText =
            "position:fixed;bottom:0;left:0;width:1px;pointer-events:none;visibility:hidden;" +
  ...
  _uploadWithProgress(url, formData) {
    return new Promise((resolve, reject) => {
      ...
      if (text) {
        text.textContent = t("common.uploading");
      }
